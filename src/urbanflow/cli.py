...
log = get_logger("urbanflow.cli")

DESCRIPTION = (
    "UrbanFlow: NYC taxi demand forecasting with calibrated uncertainty "
...
def cmd_ingest(cfg, args) -> int:
    from urbanflow.data.bronze import ingest
...
def cmd_silver(cfg, args) -> int:
    from urbanflow.data.silver import build_silver
...
def cmd_gold(cfg, args) -> int:
    from urbanflow.data.gold import build_gold, load_panel, panel_summary
...
def cmd_train(cfg, args) -> int:
    from urbanflow.data.gold import load_panel
    from urbanflow.pipelines.train import persist, run_training
...
def cmd_report(cfg, args) -> int:
    ...
    from urbanflow.data.gold import load_panel
    from urbanflow.pipelines.report import generate
    from urbanflow.pipelines.train import load_artifacts, persist, run_training
    ...
    from urbanflow.pipelines.summary import write_summary
...
def cmd_all(cfg, args) -> int:
    ...
    from urbanflow.data.bronze import ingest
    from urbanflow.data.gold import build_gold, load_panel
    from urbanflow.data.silver import build_silver
    from urbanflow.pipelines.report import generate
    from urbanflow.pipelines.train import persist, run_training
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="urbanflow", description=DESCRIPTION)
...
def main(argv: list[str] | None = None) -> int:
    ...
    log_event(log, "urbanflow start", command=args.command, overrides=args.overrides)
