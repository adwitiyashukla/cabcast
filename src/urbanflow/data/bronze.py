...
from urbanflow.data.synth import month_bounds, month_range, write_synthetic_bronze
...
def fetch_zone_reference(cfg) -> tuple[Path, Path]:
    ...
    from urbanflow.geo.zones import find_shapefile
...
def adopt_manual_downloads(cfg) -> int:
    ...
    if not drop.is_absolute():
        from urbanflow.config import ROOT
